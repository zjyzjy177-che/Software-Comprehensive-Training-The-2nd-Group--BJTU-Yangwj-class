...
LANG_TEXTS = {
    "zh_CN": {
        ...
        "first_login_hint": "‼️重要‼️：第一次登录请先注册",
        "not_logged_in": "未登录",
        "logged_in": "已登录",
    ...
    "zh_TW": {
        ...
        "first_login_hint": "‼️重要‼️：第一次登錄請先註冊",
        "not_logged_in": "未登錄",
        "logged_in": "已登錄",
    ...
    "en": {
        ...
        "first_login_hint": "‼️Important‼️: Please register first if it's your first time logging in.",
...
class BJTUSimulationGUI:
    ...
    def _build_ui(self):
        ...
        x = (sw - 600) // 2
        y = (sh - 670) // 2
        self.root.geometry(f"600x670+{x}+{y}")
        ...
        self.status_bar = tk.Label(self.root, text=self.t("not_logged_in"), font=("微软雅黑", 8),
        ...
        self.copyright_bar = tk.Label(self.root,
            ...
            font=("微软雅黑", 7), bg=self.BG, fg=self.BLUE_DARK, anchor="center")
    ...
    def _build_login_frame(self):
        ...
        tk.Label(lang_frame, text=self.t("lang_label") + ":", font=("微软雅黑", 8),
                 bg=self.BLUE, fg=self.BLUE_LIGHT).pack(side="left", padx=(0, 4))
        self._lang_btns = {}
        for code, label in [("zh_CN", "简"), ("zh_TW", "繁"), ("en", "EN")]:
            btn = tk.Button(lang_frame, text=label, font=("微软雅黑", 7),
                            command=lambda c=code: self._switch_lang(c),
                            bg=self.BLUE_DARK, fg=self.WHITE,
                            activebackground=self.BLUE_HOVER, activeforeground=self.WHITE,
                            relief="flat", bd=0, cursor="hand2", width=3, height=1)
            btn.pack(side="left", padx=1)
            self._lang_btns[code] = btn
        self._lang_btns["zh_CN"].config(bg=self.BLUE_HOVER)

        # ---- 白色登录卡片（居中） ----
        card_wrapper = tk.Frame(self.login_frame, bg=self.BG)
        card_wrapper.pack(fill="x", pady=(15, 0))
        card = tk.Frame(card_wrapper, bg=self.WHITE, relief="solid", bd=1, width=440, height=375)
        card.pack()
        ...
        # 角色选择
        self.lbl_role = tk.Label(form, text=self.t("role_label"), font=("微软雅黑", 11),
        ...
        # 账号
        self.lbl_account = tk.Label(form, text=self.t("account_label"), font=("微软雅黑", 11),
                                    bg=self.WHITE, fg=self.TEXT)
        self.lbl_account.grid(row=1, column=0, padx=(0, 10), pady=6, sticky="e")
        self.entry_account = tk.Entry(form, font=("微软雅黑", 11), width=22,
                                      relief="solid", bd=1, highlightthickness=0)
        self.entry_account.grid(row=1, column=1, pady=6, sticky="w")
        self._style_entry(self.entry_account)

        # 密码
        self.lbl_password = tk.Label(form, text=self.t("password_label"), font=("微软雅黑", 11),
                                     bg=self.WHITE, fg=self.TEXT)
        self.lbl_password.grid(row=2, column=0, padx=(0, 10), pady=6, sticky="e")
        self.entry_password = tk.Entry(form, font=("微软雅黑", 11), width=22,
        ...
        # 验证码
        self.lbl_captcha = tk.Label(form, text=self.t("captcha_label"), font=("微软雅黑", 11),
                                    bg=self.WHITE, fg=self.TEXT)
        ...
        self.entry_captcha = tk.Entry(captcha_row, font=("微软雅黑", 11), width=10,
                                      relief="solid", bd=1, highlightthickness=0)
        ...
        self.btn_local = tk.Button(btn_row, text=self.t("login_btn"), command=self._local_login,
                                   font=("微软雅黑", 11, "bold"),
                                   bg=self.BLUE, fg=self.WHITE,
                                   activebackground=self.BLUE_HOVER,
                                   activeforeground=self.WHITE,
                                   width=12, height=1, relief="flat", bd=0, cursor="hand2")
        self.btn_local.pack(side="left", padx=5)
        self._bind_hover(self.btn_local, self.BLUE, self.BLUE_HOVER)
        ...
        self.btn_register = tk.Button(link_row, text=self.t("register_btn"), command=self._register,
                                      font=("微软雅黑", 9), bg=self.WHITE, fg=self.BLUE,
        ...
        self.lbl_pwd_hint = tk.Label(card, text=self.t("default_pwd_hint"),
                                     font=("微软雅黑", 8), fg=self.HINT, bg=self.WHITE)
        self.lbl_pwd_hint.pack(pady=(8, 0))

        self.lbl_first_login = tk.Label(card, text=self.t("first_login_hint"),
                                        font=("微软雅黑", 8, "bold"), fg="#dd0000", bg=self.WHITE)
        self.lbl_first_login.pack(pady=(4, 14))

        # ---- 四张图片（排在登录卡片下方） ----
        img_frame = tk.Frame(self.login_frame, bg=self.BG)
        img_frame.pack(pady=(15, 6))
        self._pic_labels = []
        for i in range(1, 5):
            ...
            if os.path.exists(pic_path):
                ...
                lbl = tk.Label(img_frame, image=photo, bg=self.WHITE, relief="solid", bd=1)
                lbl.image = photo
    ...
    def _build_config_frame(self):
        ...
        self.info_user_label = tk.Label(ib, text=self.t("current_user_none"),
                                        font=("微软雅黑", 10), bg=self.BLUE_LIGHT, fg=self.BLUE)
        self.info_user_label.pack(side="left", padx=10, pady=4)
        self.btn_logout = tk.Button(ib, text=self.t("logout_btn"), command=self._back_to_login,
                                    font=("微软雅黑", 9), bg=self.BLUE, fg=self.WHITE, width=6)
        self.btn_logout.pack(side="right", padx=5, pady=2)

        # 仿真参数
        self.lf_params = tk.LabelFrame(self.config_scrollable, text=self.t("sim_params"),
                                       font=("微软雅黑", 12, "bold"), bg=self.BG,
        ...
        for i, (tkey, attr, default) in enumerate(params):
            lbl = tk.Label(self.lf_params, text=self.t(tkey), font=("微软雅黑", 11), bg=self.BG)
            lbl.grid(row=i, column=0, padx=5, pady=6, sticky="e")
            self._config_param_labels.append((lbl, tkey))
            entry = tk.Entry(self.lf_params, font=("微软雅黑", 11), width=14, relief="solid", bd=1)
        ...
        ttk.Combobox(self.lf_strategy, textvariable=self.strategy_var,
                     ...
                     state="readonly", font=("微软雅黑", 11), width=14).grid(
        ...
        alpha_slider = tk.Scale(self.lf_weight, from_=0.0, to=1.0, resolution=0.1,
                                ...
                                bg=self.BG, font=("微软雅黑", 9))
        alpha_slider.grid(row=0, column=1, padx=5, pady=6, sticky="w")
        self.alpha_label = tk.Label(self.lf_weight, text="0.5", font=("微软雅黑", 11), bg=self.BG, width=5)
        ...
        self.beta_label = tk.Label(self.lf_weight, text="0.5", font=("微软雅黑", 11), bg=self.BG, width=5)
        ...
        self.cb_viz = tk.Checkbutton(self.lf_other, text=self.t("enable_viz"),
                                     variable=self.enable_viz_var, font=("微软雅黑", 11),
                                     bg=self.BG)
        self.cb_viz.grid(row=0, column=0, columnspan=2, padx=5, pady=5, sticky="w")

        self.config_path_var = tk.StringVar()
        self.btn_browse = tk.Button(self.lf_other, text=self.t("browse_config"),
        ...
        self.lbl_config_path = tk.Label(self.lf_other, textvariable=self.config_path_var,
                                        font=("微软雅黑", 9), bg=self.BG, fg=self.BLUE)
        self.lbl_config_path.grid(row=1, column=1, padx=5, pady=5, sticky="w")

        # 启动按钮
        lf = tk.Frame(self.config_scrollable, bg=self.BG)
        lf.pack(fill="x", padx=10, pady=10)
        self.launch_btn = tk.Button(lf, text=self.t("launch_btn"), command=self._launch_simulation,
                                    font=("微软雅黑", 14, "bold"),
                                    bg=self.BLUE, fg=self.WHITE,
                                    activebackground=self.BLUE_HOVER,
                                    activeforeground=self.WHITE,
        ...
        self.result_text = tk.Text(self.lf_result, font=("微软雅黑", 10), wrap="word",
                                   relief="solid", bd=1, height=12)
        self.result_text.pack(fill="both", expand=True)

        self.btn_theme = tk.Button(self.config_scrollable, text=self.t("theme_btn"),
                                   command=self._toggle_theme,
                                   font=("微软雅黑", 9), bg=self.BLUE_LIGHT, fg=self.BLUE)
    ...
    def _switch_lang(self, code):
        ...
        for c, btn in self._lang_btns.items():
            btn.config(bg=self.BLUE_HOVER if c == code else self.BLUE_DARK)
    ...
    def _apply_language(self):
        ...
        # 更新语言标签
        for c, btn in self._lang_btns.items():
            btn.config(bg=self.BLUE_HOVER if c == self.lang else self.BLUE_DARK)
        ...
        self.btn_local.config(text=self.t("login_btn"))
        ...
        self.btn_logout.config(text=self.t("logout_btn"))
        ...
        self.btn_browse.config(text=self.t("browse_config"))
        self.launch_btn.config(text=self.t("launch_btn"))
        self.lf_result.config(text=self.t("result_label"))
        self.btn_theme.config(text=self.t("theme_btn"))
    ...
    def _launch_simulation(self):
        ...
        self.launch_btn.config(state="disabled", text=self.t("launching"))
    ...
    def _display_result(self, result):
        self.launch_btn.config(state="normal", text=self.t("launch_btn"))
        self.result_text.delete("1.0", tk.END)
    ...
    def _toggle_theme(self):
        ...
        if self.is_dark:
            bg, fg, ebg = "#2b2b2b", "#eeeeee", "#3c3f41"
            header_bg = "#1a1a2e"
        else:
            bg, fg, ebg = self.BG, self.TEXT, self.WHITE
            header_bg = self.BLUE

        self.root.config(bg=bg)
        self.status_bar.config(bg="#1a1a2e" if self.is_dark else self.BLUE, fg="#888888" if self.is_dark else self.WHITE)

        for frame in [self.login_frame, self.config_frame]:
            frame.config(bg=bg)

        for attr in ['login_frame', 'config_frame']:
            for widget in getattr(self, attr).winfo_children():
                self._style_widget(widget, bg, fg, ebg)

    def _style_widget(self, widget, bg, fg, ebg):
        try:
            if isinstance(widget, (tk.Label, tk.Checkbutton)):
                widget.config(bg=bg, fg=fg)
            elif isinstance(widget, tk.Entry):
                widget.config(bg=ebg, fg=fg)
